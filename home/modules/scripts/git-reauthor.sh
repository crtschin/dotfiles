...
# Build the amend command that rewrites the author email without touching author date
amend_cmd="git commit --amend --no-edit --author=\"$(git config user.name) <$(git config user.email)>\""

# Run git rebase with -x to execute the amend on each commit
if ! git rebase -x "$amend_cmd" "$@"; then
...
if [ "$needs_pop" = true ]; then
  if ! git stash pop; then
    ...
    echo "git-reauthor: stash pop conflicted with rebase result; reverted to original state" >&2
    exit 1
  fi
fi
